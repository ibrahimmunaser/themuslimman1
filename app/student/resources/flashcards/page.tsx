...
export const metadata = { title: "Flashcards | Resources" };
...
export default async function FlashcardsPage() {
  ...
  const progressMap = Object.fromEntries(
    progress.map((p) => [p.partNumber, p.flashcardsReviewed])
  );

  const reviewedCount = progress.filter((p) => p.flashcardsReviewed).length;
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h1 className="text-3xl font-bold text-text">Flashcards</h1>
            </div>
            <p className="text-text-secondary">
              Interactive flashcards for memorization and quick review
            </p>
          </div>

          {/* Stats */}
          <div className="p-4 rounded-xl border border-border bg-surface mb-8 inline-block">
            <p className="text-text-muted text-xs mb-1">Reviewed</p>
            <p className="text-2xl font-bold text-text">{reviewedCount}/{PARTS.length}</p>
          </div>

          <div className="space-y-3">
            {PARTS.map((part) => {
              const isReviewed = progressMap[part.partNumber] || false;
