...
function MonthlyCheckoutForm({
  ...
  const label = `Subscribe — ${formatPrice(finalPrice)}/mo`;
      ...
      <div className="lg:w-1/2 bg-zinc-900/50 border-r border-zinc-800 px-6 sm:px-12 py-12 flex flex-col justify-center">
        ...
        <div className="grid grid-cols-2 gap-3 mb-8">
          ...
          >
            ...
            <span className="text-xs text-zinc-500 mt-0.5">1 learner · per month</span>
          </button>

          <button
            onClick={() => setPlanChoice("familyMonthly")}
            className={`flex flex-col p-4 rounded-xl border transition-all text-left ${
              planChoice === "familyMonthly"
                ? "border-gold/50 bg-gold/8 text-white ring-1 ring-gold/30"
                : "border-zinc-700 bg-zinc-800/50 text-zinc-400 hover:border-zinc-600"
          ...
          >
            ...
            <span className="text-2xl font-bold text-white">
              {formatPrice(PLANS.familyMonthly.price)}
            </span>
            <span className="text-xs text-zinc-500 mt-0.5">Up to 5 learners · per month</span>
          </button>
        </div>
        ...
        <div className="max-w-md w-full mx-auto">
          ...
          {userEmail ? (
            <p className="text-sm text-zinc-500 mb-7">
              Subscribing as <span className="text-zinc-300">{userEmail}</span>
            </p>
          ) : (
            <div className="mb-7" />
          )}
